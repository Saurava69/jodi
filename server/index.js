...
const clienid = "yourclietidS";
const clientsecret = "Your Screat";
    ...
    people.people
      ...
      .then(async (response) => {
        ...
        if (existingUser) {
          ...
          const resobject = JSON.stringify({
            token: token,
...
app.post("/abandon", async (req, res) => {
  //endpoint for deleting the user from the data base
  const client = new MongoClient(uri);
  console.log("called");
  await client.connect();
  const database = client.db("app-data");
...
app.get("/exist", async (req, res) => {
  //endpoint for checking if the user with current authtoken is in the database
